...
The **Tooth Analyser** follows a simple flow:
- The **ToothAnalyserWidget** owns the UI and reads user input from the **ToothAnalyserParameterNode**.
- The widget triggers a concrete **ToothAnalyserLogic** implementation.
- Each logic class executes its algorithm and can optionally support batch processing.

This separation keeps UI wiring in `ToothAnalyser.py` and concentrates algorithms in
`ToothAnalyser/ToothAnalyserLib/`.
...
- Create a **new class** that **derives from `ToothAnalyserLogic`**.
...
- Add a **new segmentation option** in the **ParameterNode** at the top of the file by extending the
  `segmentation` choice list in `ToothAnalyserParameterNode`.
...
**ToothAnalyserParameterNode** so it appears in the UI selector.

## 4. Build a Python module
Complex algorithms should live in a **standalone Python module** under
`ToothAnalyser/ToothAnalyserLib/` so that UI and Slicer glue stay minimal.
...
- Create a new module file in `ToothAnalyser/ToothAnalyserLib/Algorithms/` (or in `tha/` if it fits that package).
- Keep the module **Slicer-agnostic**. Accept plain Python types where possible and isolate Slicer-specific IO to
  `ToothAnalyser.py` or thin adapters.
- Expose a small public API (one or two functions or a class) that your `ToothAnalyserLogic` subclass can call.
